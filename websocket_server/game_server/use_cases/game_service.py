...
class GameService:
    ...
    def player_round(self):
        ...
        for player in game_status.players:
            ...
            while action != "stand":
                ...
                bet = int(input("value to increase bet:"))
                player_new_bet = self.game_logic.bet(player=player, bet_value=bet)
    ...
    def end_round(self):
        game_status = self.cache.get_game_status()
        self.game_logic.end_round(game_status=game_status)
        print(game_status)
        self.cache.save_players_data(players=game_status.players)
        self.cache.save_game_status(game=game_status)
